...
ZPCID=$(sudo docker run -u zap -p 8090:8090 -d owasp/zap2docker-stable zap.sh -daemon -port 8090 -host 0.0.0.0 -config api.disablekey=true)
...
mv report.html ~/zap-mgmt-scripts_gh-pages/reports/wavsep-1.5-2.4.2-R-M-M.html
docker logs $ZPCID > ~/zap-mgmt-scripts_gh-pages/reports/wavsep-1.5-2.4.2-R-M-M.logs.txt

cd ~/zap-mgmt-scripts_gh-pages
git add reports

git commit -m "Latest report 2.4.2 vs wavsep"
git push origin
